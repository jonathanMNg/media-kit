...
class NativePlayer extends PlatformPlayer {
  ...
  Future<void> open(
    ...
    Future<void> function() async {
      ...
      if (playlist.any((media) {

        return media.uri.startsWith('fd://');
      })) {
        ...
        for (final media in playlist) {
          final realUri = await media.futureUri;
          buffer.writeln(realUri);
        }
  ...
  Future<void> add(Media media, {bool synchronized = true}) {
    Future<void> function() async {
      ...
      // ---------------------------------------------
      final realUrl = await media.futureUri;

      await _command(['loadfile', realUrl, 'append']);
    }
